...
def fake_drag(browser, knob, offset):
    offsets, tracks = slider_catch_easing.get_tracks(offset, 12, 'ease_out_expo')
    print(offsets)
...
def do_crack(browser):
    slice = browser.find_element('class name',"tc-fg-item tc-slider-normal")
    slice_offset = get_slice_offset(slice)
    print(slice_offset)

    images = browser.find_elements('class name',"gtc-fg-item")
    images_bg = browser.find_elements('class name',"tc-bg-img unselectable")
    ...
    knob = browser.find_element('class name',"gt_slider_knob")
